/* 
	Base loader is written by Yoshiaki Tazaki

	Class, LWOobj and VMAD chunk added by Urban Lassi
*/
#pragma once
#include "LWOobj.h"
...
#define ID_VMAD		MAKE_ID('V','M','A','D') //added by Urban Lassi
...
/*
	Work to be done:
		- support more than one VMAP, for example vertex weights for bones etc
*/
class LWOReader
{
public:
	...
	int readLWO(char *filename, LWOobj &object);
	...
	*/
	...
	int seek_pad( int size, FILE *file )
	{
		if (size > 0) fseek(file, (long)size, SEEK_CUR);
		return size;
	}

	int read_u1( U1 *vals, int num, FILE *file )
	{
		return (int) fread((void *)vals, sizeof(U1), num, file);
	}

	int read_u2( U2 *vals, int num, FILE *file )
	{
		int   i, bytesread;
	...
	int read_u4( U4 *vals, int num, FILE *file )
	{
		int   i, bytesread;
	...
	int read_i1( I1 *vals, int num, FILE *file )
	{
		return read_u1((U1 *)vals, num, file);
	}


	int read_i2( I2 *vals, int num, FILE *file )
	{
		return read_u2((U2 *)vals, num, file);
	}


	int read_f4( F4 *vals, int num, FILE *file )
	{
		return read_u4((U4 *)vals, num, file);
	}


	int read_col12( COL12 *col, int num, FILE *file )
	{
		return read_f4((F4 *)col, 3 * num, file);
	}


	int read_vec12( VEC12 *vec, int num, FILE *file )
	{
		return read_f4((F4 *)vec, 3 * num, file);
	}


	int read_vx( VX *vx, FILE *file )
	{
		int   ch, bytesread = 0;
	...
	int read_name( S0 name, FILE *file )
	{
		int   ch, bytesread = 0;
	...
	*/
	...
	U4 read_pnts(U4 nbytes, FILE *file, LWOobj &obj)
	{
		...
		{
			//insert into LWOobj
			obj.mVertices.push_back(fpt[i][0]);
			obj.mVertices.push_back(fpt[i][1]);
			obj.mVertices.push_back(fpt[i][2]);
	...
	*/
	...
	U4 read_bbox(U4 nbytes, FILE *file, LWOobj &object)
	{
		F4	bbox[6];
	...
	*/
	...
	U4 read_pols(U4 nbytes, FILE *file, LWOobj &object)
	{
		U2	  numvert, flags;
		U4	  nPols, bytesread;
		VX    vx;
		...
		while (bytesread < nbytes)
		{
			...
			nPols++;
			tempInt = numvert;
			object.mPolygons.push_back(tempInt);
	...
	*/
	...
	U4 read_ptag(U4 nbytes, FILE *file, LWOobj &object)
	{
		U2	 tag;
		U4	 nTags, bytesread;
		VX   vx;
		...
		while (bytesread < nbytes)
		{
			...
			if (strcmp(id, "SURF")==0) {
				//only SURF is relevant			
				tempInt = vx;
				object.mPolySurface.push_back(tempInt);
				tempInt = tag;
				object.mPolySurface.push_back(tempInt);
			}
			////printf("\tPOLY[%d] TAG[%d]\n", vx, tag); nTags++;
	...
	/*
		Discontinous vertex map mapping
	*/
	U4 read_vmad(U4 nbytes, FILE *file, LWOobj &object) {
		U2	 dim;
		U4	 bytesread;
		VX   vx;
		F4   value;
		...
		//printf(" DIM [%d] NAME [%s]\n", dim, name);
		LWOobj::DVertexMap vmad;
		vmad.name = name;
		vmad.dimensions = dim;

		while (bytesread < nbytes)
		{
			LWOobj::DiscontVertexMapElement vE;
			bytesread += read_vx( &vx, file );
			vE.vertexNo = vx;
			bytesread += read_vx( &vx, file );
			vE.polygonNo = vx;
			if (dim == 0) {
				...
				for (n = 0; n < (int) dim; n++) {
					...
					vE.values.push_back(value);
					//if (n+1 == dim) //printf("%f]\n", value);
					//else            //printf("%f, " , value);
				}
			}
			vmad.elements.push_back(vE);
		}
		object.mDVertexMaps.push_back(vmad);
		if (bytesread != nbytes) printf("??? %d != %d\n", bytesread, nbytes);
	...
	*/
	U4 read_vmap(U4 nbytes, FILE *file, LWOobj &object)
	{
		U2	 dim;
		U4	 bytesread;
		VX   vx;
		...
		bytesread += read_name( name, file );
		//printf(" DIM [%d] NAME [%s]\n", dim, name);
		LWOobj::VertexMap vmap;
		vmap.name = name;
		vmap.dimensions = dim;
		while (bytesread < nbytes)
		{
			LWOobj::VertexMapElement vE;
			bytesread += read_vx( &vx, file );
			vE.vertexNo = vx;
			if (dim == 0) {
				////printf("\tVERT[%d]\n", vx);
			...
			vmap.elements.push_back(vE);
		}
		object.mVertexMaps.push_back(vmap);
		if (bytesread != nbytes) printf("??? %d != %d\n", bytesread, nbytes);
	...
	*/
	U4 read_head(U4 nbytes, FILE *file)
	{
	...
	/*
		Read texture [BLOK] header
	*/
	U4 read_texture_head(U4 nbytes, FILE *file, LWOobj::Texture &texture)
	{
		U4	    bytesread = 0, type, byteshold;
		U2      size, u2[4];
		VX      vx[4];
	...
	*/
	...
	U4 read_tmap(U4 nbytes, FILE *file, LWOobj::Texture &texture)
	{
		U4	    bytesread = 0, type, byteshold;
		U2      size, u2[4];
		VX      vx[4];
		...
		case ID_CNTR:
			...
			texture.center.push_back(vec[0]);
			texture.center.push_back(vec[1]);
			texture.center.push_back(vec[2]);
			bytesread += read_vx(vx, file);
			break;
		case ID_SIZE:
			bytesread += read_vec12(&vec, 1, file);
			texture.size.push_back(vec[0]);
			texture.size.push_back(vec[1]);
			texture.size.push_back(vec[2]);
			bytesread += read_vx(vx, file);
			break;
		case ID_ROTA:
			bytesread += read_vec12(&vec, 1, file);
			texture.rotation.push_back(vec[0]);
			texture.rotation.push_back(vec[1]);
			texture.rotation.push_back(vec[2]);
			bytesread += read_vx(vx, file);
			break;
		case ID_FALL:
			bytesread += read_u2(u2, 1, file);
	...
	*/
	...
	U4 read_blok(U4 nbytes, FILE *file, LWOobj::Surface &surf)
	{
		U4	    bytesread = 0, type, byteshold;
		U2      size, u2[4];
		U1		u1[10];
		...
		switch (type)
		{
		case ID_IMAP://spawn new texture
			{
			LWOobj::Texture texture;
			init_texture(texture);
			texture.type = "IMAP";
			bytesread += read_texture_head(size, file, texture);
			surf.vecTextures.push_back(texture);
			}
			break;
		case ID_PROC:
			{
		...
		case ID_VMAP:
			bytesread += read_name(name, file);
			surf.vecTextures.back().vmap = std::string(name);
			////printf("<%s>\n", name);
			break;
		case ID_FLAG:
        case ID_AXIS:
		...
		case ID_PROJ:
			...
			switch (u2[0]) {
			case 0:	//Planar
				surf.vecTextures.back().projectionType = "PLANAR";
				break;
			case 1:	//Cylinder
				surf.vecTextures.back().projectionType = "CYL";
				break;
            case 2:	//Sphere
				surf.vecTextures.back().projectionType = "SPHERE";
				break;
			case 3:	//Cubic
				surf.vecTextures.back().projectionType = "CUBE";
				break;
			case 4:	//Front proj
				surf.vecTextures.back().projectionType = "FRONT";
				break;
			case 5:	//UV
				surf.vecTextures.back().projectionType = "UV";
				break;
			default:
				break;
			}
		...
		case ID_PIXB:
			...
			break;
		case ID_TMAP:
			bytesread += read_tmap(size, file, surf.vecTextures.back());
			break;
		case ID_IMAG:
			bytesread += read_vx(vx, file);
			surf.vecTextures.back().imageIndex = vx[0];
			////printf("<%d>\n", vx[0]);
			break;
		case ID_WRAP:
			...
			for (int i=0; i<2; i++) {
				switch(u2[i]) {
				case 0:
					surf.vecTextures.back().wrap[i] = "RESET";
					break;
				case 1:
					surf.vecTextures.back().wrap[i] = "REPEAT";
					break;
				case 2:
					surf.vecTextures.back().wrap[i] = "MIRROR";
					break;
				case 3:
					surf.vecTextures.back().wrap[i] = "EDGE";
					break;
				default:
					break;
				}
		...
		case ID_WRPW:
			bytesread += read_f4(f4, 1, file);
			surf.vecTextures.back().wrapAmount[0] = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_WRPH:
			bytesread += read_f4(f4, 1, file);
			surf.vecTextures.back().wrapAmount[1] = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_TAMP:
			bytesread += read_f4(f4, 1, file);
			surf.vecTextures.back().amplitude = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_VALU:
		...
		case ID_GRST:
			bytesread += read_f4(f4, 1, file);
			surf.vecTextures.back().gradientStart = f4[0];
			break;
		case ID_GREN:
			bytesread += read_f4(f4, 1, file);
			surf.vecTextures.back().gradientEnd = f4[0];
			break;
		case ID_COLR:
		...
		case ID_FUNC:
			...
			surf.vecTextures.back().procAlgoName = name;
			////printf("<%s> ", name);
	...
	*/
	...
	U4 read_surf(U4 nbytes, FILE *file, LWOobj &object)
	{
		U4	    bytesread = 0, type, byteshold;
		U2      size, u2[4];
		F4      f4[4];
		...
		ID4     id;
		COL12   col;
		//assume values by specification
		LWOobj::Surface surf;// = new LWOobj::Surface;		
		init_surface(surf);
		...
		case ID_COLR:
			bytesread += read_col12(&col, 1, file);
			bytesread += read_vx(vx, file);
			surf.baseColour[0] = col[0];
			surf.baseColour[1] = col[1];
			surf.baseColour[2] = col[2];
			break;
		case ID_LCOL:
			bytesread += read_col12(&col, 1, file);
			bytesread += read_vx(vx, file);
		...
		case ID_REFL:
			bytesread += read_f4(f4, 1, file);
			surf.reflectivity = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_TRAN:
			bytesread += read_f4(f4, 1, file);
			surf.transparency = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_TRNL:
			bytesread += read_f4(f4, 1, file);
			surf.translucency = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_GLOS:
			bytesread += read_f4(f4, 1, file);
		...
		case ID_RSAN:
			bytesread += read_f4(f4, 1, file);
			surf.reflectionMapImageSeamAngle = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_RIND:
			bytesread += read_f4(f4, 1, file);
			surf.indexOfRefraction = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_CLRH:
			bytesread += read_f4(f4, 1, file);
			surf.specularColourHighlight = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_CLRF:
			bytesread += read_f4(f4, 1, file);
			surf.colourFilter = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_ADTR:
			bytesread += read_f4(f4, 1, file);
			surf.additiveTransparency = f4[0];
			bytesread += read_vx(vx, file);
			break;
		case ID_GVAL:
		case ID_LSIZ:
		...
		case ID_AVAL:
			...
			////printf("<%f>\n", f4[0]);
			break;
		case ID_BLOK:	//branch on BLOK?!
			//printf("\n");
			bytesread += read_blok(size, file, surf);
			break;
		case ID_CMNT:
			memset( s0, 0x00, sizeof(s0) );
			bytesread += read_u1( (U1*) &s0, size, file);
			////printf("<%s>\n", s0);
			break;
		default:
			bytesread += seek_pad(size, file);
		...
		object.mSurfaces.push_back(surf);
		return(bytesread);
	}
	...
	*/
	...
	U4 read_clip(U4 nbytes, FILE *file, LWOobj &object)
	{
	...
	*/
	...
	U4 read_envl(U4 nbytes, FILE *file)
	{
	...
	*/
	...
	U4 read_tags(U4 nbytes, FILE *file, LWOobj &object)
	{
		U4    bytesread = 0L, n = 0;
		S0    name;
	...
	*/
	U4 read_layr(U4 nbytes, FILE *file)
	{
		U4       bytesread = 0L;
		U2       flags[2], parent[1];
		VEC12    pivot;
	...
	/*
		Stuff to init data carriers in LWOobj
	*/
	int init_texture(LWOobj::Texture &tex) {
		tex.amplitude = 1.0;
		tex.channel = "";
		tex.gradientEnd = 0.0;
		tex.gradientStart = 0.0;
		tex.imageIndex = 0;
		tex.procAlgoName = "";
		tex.projectionType = "";
		tex.type = "";
		tex.vmap = "";
		tex.wrap[0] = tex.wrap[1] = "";
		tex.wrapAmount[0] = tex.wrapAmount[1] = 1;
		return 1;
	}
	int init_surface(LWOobj::Surface &s) {
		s.name = "default";
		s.additiveTransparency = 0.0;
		s.alphaMode = 0;
		s.alphaValue = 1.0;
		s.baseColour[0] = s.baseColour[1] = s.baseColour[2] = 1.0;
		s.bumpiness = 0.0;
		s.colourFilter = 0.0;
		s.diffuse = 1.0;
		s.diffuseSharpness = 1.0;
		s.gloss = 32.0;
		s.indexOfRefraction = 1.0;
		s.luminance = 0.0;
		s.maxSmoothingAngle = 0.0;
		s.reflectionMapImageSeamAngle = 0.0f;
		s.reflectivity = 0.0;
		s.sidedness = 1;
		s.specular = 0.0;
		s.specularColourHighlight = 0.0;
		s.translucency = 0.0;
		s.transparency = 0.0;
		return 1;
	}
